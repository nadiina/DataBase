...
def print_data(nums: list[int], rows):
    ...
    for col in range(len(lengths)):
        for row in rows:
            # Получение значения атрибута объекта по его имени
            value = getattr(row, d[col])
            rls.append(3 if type(value) is not str else len(str(value)))
        lengths[col] = max([lengths[col]] + rls)
        rules.append("=" * lengths[col])
    format = " ".join(["%%-%ss" % l for l in lengths])
    result = [format % tuple(names), format % tuple(rules)]
    for row in rows:
        # Получение и форматирование значений атрибутов
        formatted_values = [getattr(row, attribute) for attribute in d]
        result.append(format % tuple(formatted_values))
    return "\n".join(result) + '\n'
